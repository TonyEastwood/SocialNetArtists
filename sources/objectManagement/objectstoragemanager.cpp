...
bool ObjectStorageManager::addNewObject(Object3d newObject) {
  ...
  if (newObjectFile.exists())
    newObjectFile.setFileName(
        findUniqueFileName(objectStorageDir + '/' + newObject.getName()));

  if (newObjectFile.open(QIODevice::WriteOnly)) {
    newObjectFile.write(
        newObject.getName() + ' ' +
        QByteArray::number(newObject.getQuantityVertex3d()) + ' ' +
...

    {
      ...
      std::vector<primitives::Point3d> temp =
          newObject.getVertex3dData();  // get all vertex from object
      for (auto i : temp)
        newObjectFile.write('\n' + QByteArray::number(i.x) + ' ' +
                            QByteArray::number(i.y) + ' ' +
...

    {
      ...
      std::vector<primitives::Line3d> temp =
          newObject.getLines3dData();  // get all vertex from object
      for (auto i : temp)
...

    {
      ...
      std::vector<primitives::Face3d> temp =
          newObject.getFaces3dData();  // get all vertex from object
      for (auto i : temp)
        newObjectFile.write('\n' + QByteArray::number(i.indexVertex1) + ' ' +
                            QByteArray::number(i.indexVertex2) + ' ' +
                            QByteArray::number(i.indexVertex3) + ' ' +
                            QByteArray::number(i.indexVertex4));
    }
...
Object3d ObjectStorageManager::getObjectFromListByName(QString objectName) {
  ...
  if (!objectFile.exists()) {
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "File non exist "
             << objectFile;
    return Object3d();
  }
  if (objectFile.open(QIODevice::ReadOnly)) {
    ...
    // read all vertex from file and write it to object
    {
      ...
      for (uint i = 0; i < quantityVertex;
           i++)  // parse all lines that contain vertex
      {
        currentLine = objectFile.readLine();
        tempValues = currentLine.trimmed().split(
            ' ');  // parse each line on vertex coords
        if (tempValues.size() != 3) {
          qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "Vertex size!=3";
          objectFile.close();
          return Object3d();
        }
        objectResult.addVertex3d(
            {tempValues[0].toFloat(), tempValues[1].toFloat(),
    ...
    {
      uint quantityLines =
          values[2].toUInt();  // get quantity of lines that contain line data

      for (uint i = 0; i < quantityLines;
           i++)  // parse all lines that contain line data
      {
        currentLine = objectFile.readLine();
        ...
        objectResult.addLine3d(
            {tempValues[0].toUInt(),
             tempValues[1].toUInt()});  // add line to object array
      }
    }
      ...
      for (uint i = 0; i < quantityTriangles; i++)  // parse data on triangles
      {
        ...
        objectResult.addTriangle3d(
            {0, tempValues[0].toUInt(), tempValues[1].toUInt(),
             tempValues[2]
                 .toUInt()});  // need to add normal as first element!!!
      }
    }
    ...
    qDebug() << __FILE__ << __LINE__ << __FUNCTION__ << "Can't open the file "
             << objectFile;
  return Object3d();
}

Object3d ObjectStorageManager::getObjectFromListByIndex(uint objectIndex) {
  if (objectIndex >= objectsList.size()) return Object3d();
...
void ObjectStorageManager::Initialize() { loadObjectsToList(); }

void ObjectStorageManager::loadObjectsToList() {
  if (!QDir(objectStorageDir).exists()) return;
