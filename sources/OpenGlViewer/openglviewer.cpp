...
OpenGlViewer::OpenGlViewer(Object3d _object, QWidget *parent)
  ...
  maxOrigin = drawObject.getMaxOriginAxis();  // get max abs value current
                                              // object among axis x, y, z
...
void OpenGlViewer::paintGL() {
  ...
  glOrtho(-scaleWheel * maxOrigin, scaleWheel * maxOrigin,
          scaleWheel * maxOrigin, -scaleWheel * maxOrigin,
  ...
  glColor3f(0.0f, 8.0f, 8.0f);  // outline color (blue)
...
void OpenGlViewer::wheelEvent(QWheelEvent *event) {
  scaleWheel +=
      event->angleDelta().y() / 100;  // change scale when scroll wheel
  update();
}
