...
void OpenGlViewer::paintGL() {
  ...
  glLoadIdentity();  // load matrix
                     //    glOrtho(-openGlwidth, openGlwidth, openGlheight,
                     //    -openGlheight, -500,
                     //            500); // set matrix scope (x,y,z)
  glOrtho(
      -2 * drawObject.getMaxOriginAxis().x, 2 * drawObject.getMaxOriginAxis().x,
  ...
  std::vector<primitives::point3d> vertexData = drawObject.getVertex3dData();
  std::vector<primitives::line3d> linesData = drawObject.getLines3dData();
  std::vector<primitives::triangle3d> trianglesData =
      drawObject.getTrianglesData();
  std::vector<primitives::face3d> facesData = drawObject.getFaces3dData();
